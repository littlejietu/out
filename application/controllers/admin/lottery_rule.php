...
class Lottery_rule extends MY_Admin_Controller {
	...
	public function index()
	{
		...
		$dbprefix = $this->Lottery_rule_model->db->dbprefix;
		$page     = _get_page();
		$pagesize = 10;
		$arrParam = array();
		$arrWhere = array();
...
*/
		$tb = $dbprefix.'lottery_rule a left join '.$dbprefix.'recommend b on(b.outerid=a.id and b.kind=2)';
		$list = $this->Lottery_rule_model->fetch_page($page, $pagesize, $arrWhere, 'a.*,(b.id is not null) as isrecommend', $strOrder,$tb);
		//echo $this->db->last_query();die;
		$oSysActType = _get_config('lottery_rule');

		//分页
		$pagecfg = array();
		$pagecfg['base_url']     = _create_url('admin/lottery_rule', $arrParam);
		...
		$this->load->view('admin/lottery_rule',$result);
	}
	...
	public function add()
	{
		...
		{
			$oSysActType = _get_config('lottery_rule');
			$info = $this->Lottery_rule_model->get_info_by_id($id);
			$result = array(
				'info'=>$info,
				'oSysActType' => $oSysActType,
				);
		...
		$this->load->view('admin/lottery_rule_add', $result);
	}
			...
			if ($this->form_validation->run() === TRUE)
  			{
  				...
  				$this->Lottery_rule_model->insert($data);

				//echo '成功,<a href="/admin/aa">返回列表页</a>';
				redirect(base_url('/admin/lottery_rule'));
				exit;
  			}
  			else
  			{
	...
	function del(){
		...
		$this->Lottery_rule_model->delete_by_id($id);
		redirect( base_url('/admin/lottery_rule?page='.$page) );
